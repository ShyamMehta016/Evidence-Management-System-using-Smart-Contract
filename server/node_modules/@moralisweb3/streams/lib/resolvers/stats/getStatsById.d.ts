import { operations } from '../../generated/types';
declare const name = "GetStatsByStreamId";
declare type Name = typeof name;
declare type PathParams = operations[Name]['parameters']['path'];
declare type ApiParams = PathParams;
export declare type GetStatsByStreamIdParams = ApiParams;
export declare const getStatsById: import("@moralisweb3/api-utils").EndpointFactory<{
    streamId: string;
}, {
    streamId: string;
}, {
    totalWebhooksDelivered: number;
    totalWebhooksFailed: number;
    totalLogsProcessed: number;
    totalTxsProcessed: number;
    totalTxsInternalProcessed: number;
    createdAt?: string | undefined;
    updatedAt?: string | undefined;
}, {
    totalWebhooksDelivered: number;
    totalWebhooksFailed: number;
    totalLogsProcessed: number;
    totalTxsProcessed: number;
    totalTxsInternalProcessed: number;
    createdAt?: string | undefined;
    updatedAt?: string | undefined;
}, {
    totalWebhooksDelivered: number;
    totalWebhooksFailed: number;
    totalLogsProcessed: number;
    totalTxsProcessed: number;
    totalTxsInternalProcessed: number;
    createdAt?: string | undefined;
    updatedAt?: string | undefined;
}>;
export {};
//# sourceMappingURL=getStatsById.d.ts.map